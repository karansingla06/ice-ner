...
def isEmail(token):
    return re.match("[a-z0-9A-Z_\.-]+@[\da-zA-Z\.-]+\.[a-zA-Z\.]{2,6}",token)

def convert_case(tokens, outOfVocabularyTokenOption, wordCasingLookup, uniDist, backwardBiDist, forwardBiDist, trigramDist):
    """
    ...
    try:
        ...
        for tokenIdx in range(len(tokens)):
            ...
            if token in string.punctuation or token.isdigit() or isEmail(token):
